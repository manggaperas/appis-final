    ...
    <div class="bg-white p-5">
        <div class="flex items-center justify-between">
            ...
            <form action="#" method="get" id="edit_form">
                <div class="mb-3 flex items-center justify-start py-3">
                    <select id="id_container" class="block appearance-none w-50 bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline mr-3">
                        @foreach($data as $d)
            ...
            <a class="flex items-center rounded rounded-md bg-green-500 px-2 py-2 hover:bg-green-600 text-white" href="{{ route('container.create') }}">
...
<script>
    $('#id_container').change(function() {
        ...
        if (id_container) {
            $('#edit_form').attr('action', '/container/' + id_container + '/edit/');
            $('#delete_form').attr('action', '/container/' + id_container + '/delete/');
        }
    });
</script>